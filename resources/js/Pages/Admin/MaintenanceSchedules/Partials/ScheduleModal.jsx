...
import { Modal, InputLabel, InputError, TextInput, PrimaryButton, SecondaryButton, SearchableSelect } from '@/Components';
        ...
        <Modal show={show} onClose={onClose} maxWidth="lg">
            <form onSubmit={onSubmit} className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {/* Menggunakan SearchableSelect yang baru */}
                    <div className="md:col-span-2">
                        <SearchableSelect
                            options={equipments}
                            value={equipments.find(eq => eq.id === data.equipment_id) || null}
                            onChange={(equipment) => setData('equipment_id', equipment ? equipment.id : '')}
                            label="Pilih Alat Kesehatan"
                            placeholder="Ketik nama alat atau nomor inventaris..."
                            displayValue={(eq) => eq ? `${eq.name} [${eq.inventory_number}] - ${eq.room?.name || 'Tanpa Ruangan'}` : ""}
                            required
                        />
